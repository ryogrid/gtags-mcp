...
Please execute this analysis systematically and provide a comprehensive refactoring plan with risk assessment.`
    ...
    async getDefinition(symbol) {
        try {
            const output = await this.runCommand('global', ['-x', symbol], { cwd: this.projectDir });
    ...
    async getReferences(symbol) {
        try {
            const output = await this.runCommand('global', ['-x', '-r', symbol], { cwd: this.projectDir });
            const lines = output.trim().split('\n').filter(line => line.trim());
            ...
            const results = [];
            for (const line of lines) {
                const [symbol, file, lineNum, code] = line.split(/\s+/, 4);
                results.push({
                    file: file,
                    line: parseInt(lineNum),
                    code: code || 'N/A'
    ...
    async searchPattern(pattern) {
        try {
            const output = await this.runCommand('global', ['-x', '-g', pattern], { cwd: this.projectDir });
            const lines = output.trim().split('\n').filter(line => line.trim());
